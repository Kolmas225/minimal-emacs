...
;;; Shell & PATH from fish
;; Keep `exec-path' and the PATH environment variable in sync with what an
;; interactive fish session would provide. Emacs copies PATH into `exec-path'
;; only once, at startup; programs Emacs spawns (LSP servers, compilation,
;; M-! / `shell-command') locate binaries through `exec-path', so changing the
;; OS environment afterwards is not enough. This also covers Emacs launched
;; through `fish -c emacs' (e.g. from a .desktop file), which would otherwise
;; inherit only a non-interactive fish environment.

(defun my/fish-getenv (variable)
  "Return VARIABLE's value inside an interactive fish, or nil.
An interactive fish is used (`-i') so that every entry in config.fish applies,
regardless of how Emacs itself was launched."
  (let ((fish (or (executable-find "fish") "/usr/bin/fish")))
    (when (file-executable-p fish)
      (with-temp-buffer
        (when (and (eq (call-process fish nil t nil
                                     "-i" "-c"
                                     (format "printf '%%s' \"$%s\"" variable))
                       0)
                   (> (buffer-size) 0))
          (string-trim (buffer-string)))))))

(defun my/fish-resync-path ()
  "Sync `exec-path' and PATH from an interactive fish.
Rerun after editing the fish configuration: \\[my/fish-resync-path]."
  (interactive)
  (when-let* ((path (my/fish-getenv "PATH")))
    (let* ((dirs (split-string path path-separator t))
           (path (string-join dirs path-separator)))
      (setq exec-path dirs)
      (setenv "PATH" path))))

(my/fish-resync-path)

;;; Infrastructure: byte-compilation manager & compatibility shim
...
;;; Built-in Emacs: core defaults, UI, editing, navigation, keybindings

(add-to-list 'default-frame-alist '(font . "Maple Mono-13"))

(let ((mono "Maple Mono") (mono-size 13)
      (cjk "LXGW WenKai Screen")
      (var-pitch "Inter") (var-size 13))
  (set-face-attribute 'default nil :font mono :height (* mono-size 10))
  (set-face-attribute 'fixed-pitch nil :font mono :height (* mono-size 10))
  (set-face-attribute 'fixed-pitch-serif nil :font mono :height (* mono-size 10))
  (set-face-attribute 'variable-pitch nil :family var-pitch :height (* var-size 10))
  (dolist (script '(cjk-misc han kana hangul))
    (set-fontset-font t script cjk)))
...
(keymap-global-set "C-<next>" #'scroll-right)
...
;;; Appearance: themes, modeline, icons, ligatures, indentation, parens
...
;;; Completion: in-buffer (corfu/cape) & templates (tempel)
...
;;; Git: magit, diff-hl, gitconfig/gitignore modes
...
;;; Terminal & sessions: vterm, easysession, buffer cleanup
...
(use-package easysession
  ...
  (easysession-frameset-restore-force-current-display (daemonp))
...
;;; LSP: eglot and its consult/eldoc companions
...
(use-package eglot
  ...
  (:map eglot-mode-map
        ...
        ("<f12> i" . #'eglot-find-implementation)
        ("<f12> t" . #'eglot-find-typeDefinition)
        ("<f12> r" . #'eglot-rename)
...
;;; Checking & formatting: flymake, apheleia
...
;;; Org mode: core config and modern display
...
(use-package python
  ...
  :init
  ;; Prefer the tree-sitter based major mode when the grammar is available.
  ...
  (defun my/eglot-dynamic-server (interactive)
    ...
    (let ((dir (locate-dominating-file default-directory "pixi.toml")))
      (cond
       (dir
        '("pixi" "run" "ty" "server"))
       (t
        '("ty" "server"))))) ; Fallback default
  
  ;; Register the function to evaluate dynamically for your major-mode
  (with-eval-after-load 'eglot
    (add-to-list 'eglot-server-programs
                 '((python-base-mode python-ts-mode) . my/eglot-dynamic-server)))
  
  ;; Format with ruff instead of the default black.
...
;;; TODO & annotations: hl-todo highlighting
